        ...
        <div class="container">
            <div class="row">
                ...
                <div class="col-lg-4 mb-20">
                    @if ($teddyBearAds->count() > 0)
                        @php
                            $ad = $teddyBearAds->first();
                            $colors = json_decode($ad->custom_colors, true);
                        @endphp
                        <div class="{{ $colors['background_class'] ?? 'block-sale-1' }} ads-space-4-item">
                            <div class="row">
                                <div class="col-lg-8 col-sm-8 col-8">
                                    @if (!empty($colors['discount_text']))
                                        <span
                                            class="font-sm {{ $colors['discount_color'] ?? 'color-danger' }} text-uppercase">{{ $colors['discount_text'] }}</span>
                                        @if (!empty($colors['discount_label']))
                                            <span
                                                class="font-sm text-uppercase {{ $colors['label_color'] ?? 'color-brand-3' }}">{{ $colors['discount_label'] }}</span>
                                        @endif
                                    @endif
                                    <h3 class="mb-10">{{ $ad->headline }}</h3>
                ...
                {{-- Wooden Toys Section --}}
                <div class="col-lg-4 mb-20">
                    @if ($woodenToysAds->count() > 0)
                        @php
                            $ad = $woodenToysAds->first();
                            $colors = json_decode($ad->custom_colors, true);
                        @endphp
                        <div class="{{ $colors['background_class'] ?? 'block-sale-1 bg-4' }} ads-space-4-item">
                            <div class="row">
                                <div class="col-lg-8 col-sm-8 col-8">
                                    @if (!empty($colors['discount_text']))
                                        <strong
                                            class="font-sm {{ $colors['discount_color'] ?? 'color-danger font-bold' }} text-uppercase">{{ $colors['discount_text'] }}</strong>
                                    @endif
                                    <h3 class="mb-10">{{ $ad->headline }}</h3>
                                    <a class="btn btn-brand-2 btn-arrow-right"
                                        href="{{ $ad->call_to_action_url ?? '#' }}"
                    ...
                    @else
                        {{-- Empty Ad Space for Wooden Toys --}}
                ...
                <div class="col-lg-4 mb-20">
                    @if ($babyProductsAds->count() > 0)
                        @php
                            $ad = $babyProductsAds->first();
                            $colors = json_decode($ad->custom_colors, true);
                        @endphp
                        <div class="{{ $colors['background_class'] ?? 'block-sale-1 bg-10' }} ads-space-4-item">
                            <div class="row">
                                <div class="col-lg-8 col-sm-8 col-8">
                                    @if (!empty($colors['discount_text']))
                                        <strong
                    ...
                    @else
                        {{-- Empty Ad Space for Baby Products --}}
