    ...
    function clearBrowsingHistory() {
        showCustomDialog({
            ...
            onConfirm: function() {
                ...
                form.method = 'POST';
                form.action = '{{ route('buyer.dashboard.browsing-history.clear') }}';
...
@section('dashboard-content')
    <div class="dashboard-header d-flex justify-content-between align-items-center">
        ...
        @if ($history->count() > 0)
            <form action="{{ route('buyer.dashboard.browsing-history.clear') }}" method="POST" class="d-inline">
                @csrf
                @method('DELETE')
                    ...
                    <div class="product-image">
                        @if ($item->product)
                            ...
                            <div class="product-overlay">
                                <a href="{{ route('buyer.product.show', $item->product) }}" class="btn btn-sm btn-primary">
                    ...
                    <div class="product-info">
                        @if ($item->product)
                            ...
                            <p class="product-price">
                                {{ number_format($item->product->price_discounted > 0 ? $item->product->price_discounted : $item->product->price_regular, 2) }}
                                DJF</p>
                            @if ($item->product->price_discounted > 0)
                                <p class="product-discount">
                                    <span class="original-price">{{ number_format($item->product->price_regular, 2) }}
                                        DJF</span>
                                    <span class="discount-price">{{ number_format($item->product->price_discounted, 2) }}
                                        DJF</span>
                                </p>
                            @endif
                        ...
                        @endif
                        <p class="viewed-date">
                            <i class="fas fa-clock"></i> Viewed {{ $item->viewed_at->diffForHumans() }}
                        </p>
                    </div>
