...
<script>
        ...
        document.querySelectorAll('.remove-wishlist-btn').forEach(button => {
            button.addEventListener('click', function() {
                ...
                showCustomDialog({
                    ...
                    type: 'warning',
                    onConfirm: function() {
                        performRemoveFromWishlist(wishlistId, button);
                    }
                });
            });
        });
    ...
    function performRemoveFromWishlist(wishlistId, buttonElement) {
...
@section('dashboard-content')
            ...
            @foreach ($wishlistItems as $wishlistItem)
                <div class="wishlist-card">
                    ...
                    <div class="product-info">
                        @if ($wishlistItem->product)
                            ...
                            <p class="product-price">
                                {{ number_format($wishlistItem->product->price_discounted > 0 ? $wishlistItem->product->price_discounted : $wishlistItem->product->price_regular, 2) }}
                                DJF</p>
                            @if ($wishlistItem->product->price_discounted > 0)
                                <p class="product-discount">
                                    <span
                                        class="original-price">{{ number_format($wishlistItem->product->price_regular, 2) }}
                                        DJF</span>
                                    <span
                                        class="discount-price">{{ number_format($wishlistItem->product->price_discounted, 2) }}
                                        DJF</span>
                                </p>
                            @endif
                            <p class="added-date">Added {{ $wishlistItem->created_at->diffForHumans() }}</p>
                        @else
                    ...
                    <div class="product-actions">
                        ...
                        @endif
                        <button type="button" class="btn btn-sm btn-outline-danger remove-wishlist-btn"
                            data-wishlist-id="{{ $wishlistItem->id }}"
                            data-product-title="{{ $wishlistItem->product->title ?? 'this item' }}">
    ...
    @else
        <div class="empty-state">
            ...
            <p class="text-muted">Save items you love to your wishlist and never lose track of them!</p>
