...
<head>
    ...
    <!-- Page Title  -->
    <title>403 Forbidden - DjibMarket</title>
    <!-- StyleSheets  -->
    ...
    <style>
        .custom_guard::before {
            ...
            opacity: .3;
            z-index: -1;
            /* Ensure it's behind the shapes */
        }

        .custom_guard {
            position: relative;
            background: #e8534714;
            /* Adjusted color for error */
            color: #e85347;
            /* Adjusted color for error */
            padding: .4rem;
            text-align: center;
            ...
            /* Adjusted color for error */
            overflow: hidden;
            /* Keep ::before pseudo-element contained */
        }
                            ...
                            <div class="card-inner card-inner-lg">
                                <div class="nk-block-head">
                                    <div class="nk-block-head-content text-center">
                                        <h4 class="nk-block-title">Access Forbidden</h4>
                                        <div class="nk-block-des">
                                            <p>You don't have permission to access this resource. Please contact your
                                                administrator or make sure you are properly authenticated.</p>
                                        </div>
                                    </div>
                                </div>
                                <div class="form-group text-center">
                                    ...
                                    @if (Auth::guard('seller')->check())
                                        <a href="{{ route('seller.dashboard') }}"
                                            class="btn btn-lg btn-primary @if (Auth::guard('admin')->check()) ms-2 @endif">
                                            <em class="icon ni ni-dashboard"></em>
                                            <span>Seller Dashboard</span>
                                        </a>
                                    @endif
                                    @if (Auth::guard('web')->check())
                                        {{-- Assuming a standard 'dashboard' route exists for the 'web' guard --}}
                                        <a href="{{ route('buyer.home') }}"
                                            class="btn btn-lg btn-primary @if (auth('admin')->check() || auth('seller')->check()) ms-2 @endif">
                                            <em class="icon ni ni-dashboard"></em>
                                            <span>Home</span>
                                        </a>
                                    @endif

                                    {{-- Always show Go Home as a fallback --}}
                                    <a href="{{ url('/') }}" class="btn btn-lg btn-outline-primary ms-2">
