...
!(function (NioApp, $) {
    ...
    // Dark Mode Switch @since v2.0 - Modified for Cookie Persistence & Early Application
    NioApp.ModeSwitch = function () {
        var toggle = $(".dark-switch");
        var htmlElement = document.documentElement; // Reference HTML element

        // Synchronize body class and switch state with HTML element on load
        if (htmlElement.classList.contains("dark-mode")) {
            $body.addClass("dark-mode"); // Ensure body has class if needed
            toggle.addClass("active");
        } else {
            $body.removeClass("dark-mode"); // Ensure body does not have class
            toggle.removeClass("active");
        }

        // Attach click handler (logic remains largely the same)
        toggle.on("click", function (e) {
            e.preventDefault();
            var $this = $(this);
            // Determine the NEW state based on the HTML element's current class
            var isDarkMode = !htmlElement.classList.contains("dark-mode");

            // Toggle classes on HTML, Body, and Switch
            if (isDarkMode) {
